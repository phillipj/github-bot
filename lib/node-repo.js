...
function deferredResolveLabelsThenUpdatePr (options) {
  ...
  return new Promise(resolve => setTimeout(resolve, timeoutMillis))
    .then(() => resolveLabelsThenUpdatePr(options))
}

function resolveLabelsThenUpdatePr (options) {
  const getFiles = async () => {
    options.logger.info('Fetching PR files for labelling')

    return githubClient.pullRequests.getFiles({
      owner: options.owner,
      repo: options.repo,
      number: options.prId
    })
  ...
  return retry({ times: 5, interval: fiveSeconds }, getFiles)
    .then((res) => {
...
function fetchExistingThenUpdatePr (options, labels) {
  return fetchExistingLabels(options).then((existingLabels) => {
...
function fetchExistingLabels (options) {
  const cacheKey = `${options.owner}:${options.repo}`

  if (existingLabelsCache.has(cacheKey)) {
    return Promise.resolve(existingLabelsCache.get(cacheKey))
  }

  return fetchLabelPages(options, 1).then((existingLabels) => {
...
function fetchLabelPages (options, startPageNum) {
  ...
  return githubClient.issues.getLabels({
    ...
    return fetchLabelPages(options, startPageNum + 1)
      .then((remainingLabels) => existingLabels.concat(remainingLabels))
  })
}
